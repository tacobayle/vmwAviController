...
resource "null_resource" "ansible" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook ansible/main.yml --extra-vars '{\"avi_backup_passphrase\": ${jsonencode(var.avi_backup_passphrase)}, \"controller\": ${jsonencode(var.controller)}, \"avi_password\": ${jsonencode(var.avi_password)}, \"avi_version\": ${split("-", basename(var.contentLibrary.file))[1]}}'"
  }
}